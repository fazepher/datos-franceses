...
legis_07 <- read_csv2(file = "DATOS_BRUTOS/LG07_Bvot_T1T2_FAZH.txt", 
                     col_names = c("VUELTA",
...
legis_07 <- read_delim(file = "DATOS_ADMINISTRATIVOS/depts2007.txt",
                         delim = "\t",locale = locale(encoding = "latin1")) %>%
  transmute(COD_REGION = REGION,COD_DEPARTAMENTO = DEP, NOM_DEPARTAMENTO = NCCENR) %>%
  left_join(legis_07,., by = "COD_DEPARTAMENTO")

# Agregamos las Regiones Antiguas
legis_07 <- read_delim(file = "DATOS_ADMINISTRATIVOS/reg2007.txt",
                         delim = "\t",locale = locale(encoding = "latin1")) %>%
  transmute(COD_REGION = REGION, NOM_REGION = NCCENR) %>%
  left_join(legis_07,., by = "COD_REGION")

# Agregamos los nombres de las Regiones Nuevas
legis_07 <- read_delim(file = "DATOS_ADMINISTRATIVOS/AUXILIAR_REG_DEPART.csv",
                         delim = ",",locale = locale(encoding = "latin1")) %>%
  select(-NOM_REGION) %>%
  left_join(legis_07,., by = "COD_REGION")
...
ERRORES_COMUNAS <- anti_join(legis_07,IMG1_07,by="CODGEO") %>% 
...
# Corregimos el error del CODGEO en los resultados electorales con el correcto. 
...
legis_07 <- legis_07 %>% 
  mutate(CODGEO = if_else(NOM_COMUNA == "Han-devant-Pierrepont","54602",CODGEO),
         COD_COMUNA = if_else(NOM_COMUNA == "Han-devant-Pierrepont","602",COD_COMUNA),
         NOM_DEPARTAMENTO = if_else(NOM_COMUNA == "Han-devant-Pierrepont","Meurthe-et-Moselle",NOM_DEPARTAMENTO),
         COD_DEPARTAMENTO = if_else(NOM_COMUNA == "Han-devant-Pierrepont","54",COD_DEPARTAMENTO))

# Comunas que están en el censo, pero no en los resultados electorales y que sí existían en 2007
ERRORES_COMUNAS <- anti_join(IMG1_07,legis_07,by="CODGEO") %>% 
  select(CODGEO,NIVEL) %>% 
  distinct %>%
  semi_join(COMUNAS_07,by="CODGEO") %>% 
  full_join(ERRORES_COMUNAS)
...
VUELTA_1_LEGIS_07 <- legis_07 %>%
  select(CODGEO,INSCRITOS:VOT_CANDIDATO) %>%
  group_by(CODGEO,ETIQUETA,APELLIDO_CANDIDATO) %>%
  summarise_if(is.numeric,funs(sum(.))) %>%
  ungroup %>% 
  left_join(read_csv("BASES_CANDIDATOS/FAMILIAS_POLITICAS_07_LEGISLATIVAS.csv",
...
LEGISLATIVAS_2007 <- list(BASE_COMUNAS = BASE_COMUNAS,
                          ERRORES_COMUNAS = ERRORES_COMUNAS,
                          CENSO=IMG1_07,
                          VUELTA_1=VUELTA_1_LEGIS_07)
save(LEGISLATIVAS_2007,file = "DATOS_PROCESADOS/LEGISLATIVAS_2007.RData")
