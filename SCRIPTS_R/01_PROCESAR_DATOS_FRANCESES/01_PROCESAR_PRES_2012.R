...
pres_12 <- read_csv2(file = "DATOS_BRUTOS/PR12_Bvot_T1T2_FAZH.txt", 
                     col_names = c("VUELTA",
...
pres_12 <- read_delim(file = "DATOS_ADMINISTRATIVOS/depts2012.txt",
                         delim = "\t",locale = locale(encoding = "latin1")) %>%
  transmute(COD_REGION = REGION,COD_DEPARTAMENTO = DEP, NOM_DEPARTAMENTO = NCCENR) %>%
  left_join(pres_12,., by = "COD_DEPARTAMENTO")

# Agregamos las Regiones Antiguas
pres_12 <- read_delim(file = "DATOS_ADMINISTRATIVOS/reg2012.txt",
                         delim = "\t",locale = locale(encoding = "latin1")) %>%
  transmute(COD_REGION = REGION, NOM_REGION = NCCENR) %>%
  left_join(pres_12,., by = "COD_REGION")

# Agregamos los nombres de las Regiones Nuevas
pres_12 <- read_delim(file = "DATOS_ADMINISTRATIVOS/AUXILIAR_REG_DEPART.csv",
                         delim = ",",locale = locale(encoding = "latin1")) %>%
  select(-NOM_REGION) %>%
  left_join(pres_12,., by = "COD_REGION")
...
ERRORES_COMUNAS <- anti_join(pres_12,IMG2A_12,by="CODGEO") %>% 
...
# Comunas que están en los resultados electorales, pero no en el censo y que sí existían en 2014
ERRORES_COMUNAS <- anti_join(pres_12,IMG2A_12,by="CODGEO") %>% 
...
# Comunas que están en los resultados electorales, pero no en el censo y que NO existían NI en 2012 NI en 2014
ERRORES_COMUNAS <- anti_join(pres_12,IMG2A_12,by="CODGEO") %>% 
  select(CODGEO,NOM_COMUNA,NOM_DEPARTAMENTO) %>% 
  distinct %>% 
  full_join(ERRORES_COMUNAS)
...
ERRORES_COMUNAS <- anti_join(IMG2A_12,pres_12,by="CODGEO") %>% 
  select(CODGEO,NIVEL) %>% 
  distinct %>%
  semi_join(COMUNAS_12,by="CODGEO") %>% 
  full_join(ERRORES_COMUNAS)
...
PRESIDENCIALES_2012 <- list(BASE_COMUNAS = BASE_COMUNAS,
                            ERRORES_COMUNAS = ERRORES_COMUNAS,
                            CENSO=IMG2A_12,
                            VUELTA_1=VUELTA_1_PRES_12)
save(PRESIDENCIALES_2012,file = "DATOS_PROCESADOS/PRESIDENCIALES_2012.RData")
